...
document.addEventListener('DOMContentLoaded', (event) => {
  ...
  async function getLike() {
    try {
      ...
      console.log(likes);
      const tbody = document.querySelector('#like-list tbody'); // 좋아요 정보를 표시할 테이블의 tbody 선택
      tbody.innerHTML = '';
      likes.map(function (like) {
        ...
        td = document.createElement('td'); // 새로운 td 요소 생성
        td.textContent = like.liked ? '좋아요' : '아니에요'; // like.liked 값 사용
        row.appendChild(td);

        // 'row' 요소를 'tbody' 요소에 추가
        tbody.appendChild(row);
      });
    } catch (err) {
      console.error(err);
  ...
  if(likeForm) {  
    ...
    try {
      ...
      await axios.post('/likes', { id, liked });
